...
var App, closeThemeBox, copyCurrentTheme, dark_theme, dark_tooth, elp, emacsColors, fromEmacsColor, generateDeftheme, getColor, getFaceList, importThemeHandler, light_theme, loadConfig, masterKeys, removeTheme, saveCurrentTheme, saveToLocalStorage, setColor, setTheme, setUndoKey, setUndoLive, themeGenerator, themes, undo, undo_theme, updateUserThemes, user_themes;
...
copyCurrentTheme = function() {
  var Copied;
  Copied = App.generated_theme.createTextRange();
  return Copied.execCommand("Copy");
};

saveCurrentTheme = function() {
  var download_link, text_blob;
  if (!(App.generated_theme && App.generated_theme_name)) {
    return;
  }
  text_blob = new Blob([App.generated_theme], {
    type: 'text/plain'
  });
...
setTheme = function(theme_json, name) {
  var o;
  if (name == null) {
    name = null;
  }
...
updateUserThemes = function() {
  $('#user-themes').empty();
  _.each(_.keys(user_themes), function(k) {
    return delete user_themes[k];
  });
...
setUndoLive = function() {
  return this.undo_theme = JSON.stringify(App.live_theme);
};

undo = function() {
  setTheme(this.undo_theme);
